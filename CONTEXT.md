...
- **command** — a single `.md` file in `skills/th/` that Claude Code exposes as a slash command under the `th:` namespace; synonymous with "skill" in this repo
...
- **TDD loop** — the red → green → refactor cycle that `/th:work` runs for every behavior
- **grilling** — asking one question at a time to resolve alignment before committing to an approach; the core technique of `/th:plan`
...
- **vertical slice** — one behavior, tested and implemented end-to-end before moving to the next; the unit of progress in `/th:work`
- **deep module** — a module with a simple interface hiding complex implementation; the architectural ideal promoted by `/th:improve-arch`
